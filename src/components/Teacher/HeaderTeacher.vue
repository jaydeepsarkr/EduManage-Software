              ...
              <div>
                ...
                >
                  ...
                  >
                    ...
                    <button
                      type="button"
                      @click="removeQualificationFile(index)"
                      class="text-red-500 hover:text-red-700 text-sm"
                    >
                      Remove file
                    </button>
...
<script setup>
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      if (!aadhaarRegex.test(currentTeacher.aadhaarNumber)) {
        alert("Please enter a valid Aadhaar number in format: XXXX XXXX XXXX");
        return;
      }

      const incompleteQualifications = qualifications.value.some(
        (q) => !q.file || !q.type || !q.institution || !q.year
      );

      if (incompleteQualifications) {
        alert(
          "Please complete all qualification details and upload certificates"
        );
        return;
      }
      ...
      Object.keys(currentTeacher).forEach((key) => {
        formData.append(key, currentTeacher[key]);
      });

      formData.append("aadhaarCard", aadhaarFile.value);

      qualifications.value.forEach((qual, index) => {
        formData.append(`qualification_${index}_type`, qual.type);
        formData.append(`qualification_${index}_institution`, qual.institution);
        formData.append(`qualification_${index}_year`, qual.year);
        if (qual.file) {
          formData.append(`qualification_${index}_file`, qual.file);
        }
      });
